...
public class CaseUsb extends BaseCase {
        
        private static final String TAG = "CaseUsb";
	...
	public class MemoryInfo {
		...
		String toOneString() {
			...
			for (int i = 0; i < storageVolumes.length; i++) {
				...
				if (volume.getPath().equals("/mnt/usbhost0")) {
					Log.e(TAG, "=======extsd=========");
					...
					if (write()) {
						writeTxt.setText("USB 存储器 数据读写成功");
	...
	private boolean write() {
		boolean state = false;
		String fileName = "/mnt/usbhost0/factoryRun";
	...
	protected void onInitialize(Node attr) {
		setView(R.layout.case_usb);
		setName(R.string.case_usb_name);
		infoTxt = (TextView) mView.findViewById(R.id.usb_info_text);
		writeTxt = (TextView) mView.findViewById(R.id.usb_write_text);
	...
	protected void onPassableChange() {
		...
		if (getPassable()) {
			getView().findViewById(R.id.usb_name).setBackgroundColor(
					Color.GREEN);

		} else {
			getView().findViewById(R.id.usb_name).setBackgroundColor(Color.RED);
		}
	}
