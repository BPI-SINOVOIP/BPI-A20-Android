...
status_t CedarXPlayer::setDataSource(const char *uri, const KeyedVector<
	...
	if (headers) {
        ...
        //-------get headers----------->
        int hdrSize = 0, i = 0;
        CdxHttpHeaderFieldsT *hfs = NULL;
        CdxHttpHeaderFieldT *hdr = NULL;
        
        hdrSize = headers->size();
        CHECK(hdrSize != 0);
        hfs = (CdxHttpHeaderFieldsT *)malloc(sizeof(*hfs));
        CHECK(hfs);
        hfs->num = hdrSize;
        hdr = (CdxHttpHeaderFieldT *)malloc(sizeof(CdxHttpHeaderFieldT) * hdrSize);
        CHECK(hdr);
        hfs->pHttpHeader = hdr;
        
        for (i = 0; i < hdrSize; i++)
        {
            hdr->key = strdup(headers->keyAt(i).string());
            hdr->val = strdup(headers->valueAt(i).string());
            hdr++;
        }
        mPlayer->control(mPlayer, CDX_CMD_SET_URL_HEADERS, (unsigned int)hfs, 0);
...
void CedarXPlayer::reset_l() {
...

	{
		...
		if (mVideoRenderer != NULL)
		{
			...
			{
				mVideoRenderer->control(VIDEORENDER_CMD_SHOW, 3);
			}
			else
			{
    ...
    
    {
        ...
        if(mVideoRenderer != NULL)
        {
            if(native == NULL)  //it means user want to clear nativeWindow
            {
				...
				mVideoRenderer->control(VIDEORENDER_CMD_KEEP_LAST_FRAME, 1);
#endif
...
int CedarXPlayer::StagefrightVideoRenderInit(int width, int height, int format, void *frame_info)
{
	...
	//set picture format for display 
	...
	//mNativeWindow pass by android apk, use for display picture
	if (mNativeWindow == NULL)
	    return -1;

    if(mRenderToDE == 1)
    {
		//display with hardware layer
        mPreRenderPattern = 1;
    }
    else
    {
		//display with android GPU combine
        mPreRenderPattern = 0;
    }
    ...
    {
		//1->90C rotation, 3->270C rotation
	...
	{
		nAppVideoWidth=mVideoWidth;
	    nAppVideoHeight=mVideoHeight;//YF
    ...
    meta->setInt32(kKeyWidth, mDisplayWidth); //mDisplayWidth as same as mAppVideoWidth, mVideoWidth
