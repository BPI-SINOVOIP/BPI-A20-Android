...
status_t ChromiumHTTPDataSource::connect_l(
    ...
    while (mState == CONNECTING || mState == DISCONNECTING) {
        status_t err = mCondition.waitRelative(mLock, 2*60*1000*1000*1000ll);//120s.
        if(err == -ETIMEDOUT) {
            LOG_PRI(ANDROID_LOG_INFO, LOG_TAG, "xxx connect to http server timeout,"
						"return %d", err);
            return ERROR_IO;
        }
        LOG_PRI(ANDROID_LOG_INFO, LOG_TAG, "xxx mState=%d", mState);
