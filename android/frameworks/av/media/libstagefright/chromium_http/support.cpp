...
void SfDelegate::OnResponseStarted(net::URLRequest *request) {
    ...
    int n;
    if (mRangeRequested && request->GetResponseCode() == 200)
    {
        if (tempOffSet == 9)
        {
            scoped_refptr<net::IOBufferWithSize> tempbuf;
            tempbuf = new net::IOBufferWithSize(9);
            request->Read(tempbuf, tempOffSet, &n);
//            delete tempbuf; 
        }
    }
