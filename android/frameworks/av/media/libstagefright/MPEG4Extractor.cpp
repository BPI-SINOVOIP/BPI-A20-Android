...
status_t MPEG4Extractor::parseChunk(off64_t *offset, int depth) {
    ...
    switch(chunk_type) {
        ...
        case FOURCC('c', 'o', '6', '4'):
        {
            ...
            {  
                err = 
                mLastTrack->sampleTable->saveChunkOffsetParams(
                        chunk_type, data_offset, chunk_data_size);
        ...
        case FOURCC('s', 't', 'z', '2'):
        {
            ...
            {
                err =
                mLastTrack->sampleTable->saveSampleSizeParams(
                        chunk_type, data_offset, chunk_data_size);
        ...
        {
            if (!mIsDrm) {
				ALOGV("line %d offest %lld, trunk %lld", __LINE__, *offset, chunk_size);
                *offset += chunk_size;
				ALOGV("line %d offest %lld, trunk %lld", __LINE__, *offset, chunk_size);
                break;
            }
