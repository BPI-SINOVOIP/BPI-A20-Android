...
//        LOGV("base:'%s', url:'%s' => '%s'", baseURL, url, out->c_str());
    ...
    if (url[0] == '/') {
        ...
        char *slashPos = strstr(baseURL, ".m3u8");
        if(slashPos != NULL)
        {
            //LOGV("*************************protocolEnd=%s\n", slashPos);
            while(slashPos >= baseURL)
            {
                if(*slashPos == '/')
                {
                    break;
...
status_t M3UParser::parse(const void *_data, size_t size) {
    ...
    while (offset < size) {
        ...
        while(data[offsetData-1] == '\r'){
        	--offsetData;
        }
        line.setTo(&data[offset], offsetData - offset);
       ...
       // LOGV("#%s#", line.c_str());
        ...
        if (lineNo == 0 && line.startsWith("#EXTM3U")) {
            mIsExtM3U = true;
        }
        ...
        if (mIsExtM3U) {
            ...
            //LOGV("xxxxxx %s xxxxx", line.c_str());
            if (line.startsWith("#EXT-X-TARGETDURATION")) {
                ...
                mIsComplete = true;
                //LOGV("xxxxxxx mIsComplete = %d xxxxxxxxx", mIsComplete);
