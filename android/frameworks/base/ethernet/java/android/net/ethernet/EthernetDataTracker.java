    ...
    private void handlePppoe (int pppoeState){
        ...
        switch (pppoeState){
            case EthernetManager.PPPOE_STATE_ENABLED:
                ...
                mLinkProperties.clear();
                // Use pppoe link info reset mLinkProperties
                String lp_data = null;
                lp_data = SystemProperties.get("net.ppp0.dns1");
                if (lp_data.isEmpty() || lp_data == null)
                    Log.d(TAG, "pppoe dns1 unset");
                else
                    mLinkProperties.addDns(NetworkUtils.numericToInetAddress(lp_data));
                lp_data = SystemProperties.get("net.ppp0.dns2");
                if (lp_data.isEmpty() || lp_data == null)
                    Log.d(TAG, "pppoe dns2 unset");
                else
                    mLinkProperties.addDns(NetworkUtils.numericToInetAddress(lp_data));
