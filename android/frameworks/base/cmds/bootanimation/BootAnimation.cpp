...
BootAnimation::BootAnimation() : Thread(false)
{
    ...
    if(access(USER_BOOTVIDEO_FILE, R_OK) == 0){
        mBootVideo = true;
    } else if(access(USER_BOOTMUSIC_FILE, R_OK) == 0){
        mBootMusic = true;
    }
}
...
void BootAnimation::startBootMedia(){
    if (mBootVideo) {
        ...
        */ 
        mPlayer->setVideoSurfaceTexture(mFlingerSurface->getSurfaceTexture());
...
status_t BootAnimation::readyToRun() {
	...
	if(mBootVideo)
	{
        ...
        this->startBootMedia();
        do {
            //Because setLoop is not usefull, so i auto check it for loop again
