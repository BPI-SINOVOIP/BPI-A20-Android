...
public class SoundController extends BroadcastReceiver{
    ...
    public SoundController(Context context){
        ...
        if(getHeadphoneAvailableState() == AudioSystem.DEVICE_STATE_AVAILABLE){
    ...
    private int getDevState(String statePath){
        try{
            ...
            Log.e(TAG, "Cound not parse switch state for " + statePath + " ,fail because " + e.getMessage());
            return 0;
        }
    }
    ...
    private void handleExternalDevice(final String name, final int type, final int state, final String extra){
        ...
        Thread thread = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                String title = null;
                String message = null;
                String systemOutputChannel = null;
                log(mng);
                switch(state){
                case AudioDeviceManagerObserver.PLUG_IN:
                    switch(type){
                    ...
                    case AudioDeviceManagerObserver.AUDIO_OUTPUT_TYPE:
                        ...
                        systemOutputChannel = null;
                        for(int i = 0; i < audio_out.size(); i++){
                            if(systemOutputChannel == null){
                                systemOutputChannel = audio_out.get(i);
                            }else{
                                systemOutputChannel = systemOutputChannel + "," + audio_out.get(i);
                            }
                        }
                        Settings.System.putString(mContext.getContentResolver(), Settings.System.AUDIO_OUTPUT_CHANNEL,systemOutputChannel);
                        Log.d(TAG,"audio channels change to {" + systemOutputChannel + "}");
                ...
                case AudioDeviceManagerObserver.PLUG_OUT:
                    ...
                    case AudioDeviceManagerObserver.AUDIO_OUTPUT_TYPE:
                        ...
                        for(int i = 0; i < olist.size(); i++){
                            if(systemOutputChannel == null){
                                systemOutputChannel = olist.get(i);
                            }else{
                                systemOutputChannel = systemOutputChannel + "," + olist.get(i);
                            }
                        }
                        Settings.System.putString(mContext.getContentResolver(), Settings.System.AUDIO_OUTPUT_CHANNEL,systemOutputChannel);
                        Log.d(TAG,"audio channels change to {" + systemOutputChannel + "}");
