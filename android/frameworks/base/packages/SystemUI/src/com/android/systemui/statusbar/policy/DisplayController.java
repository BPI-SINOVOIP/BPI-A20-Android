...
public class DisplayController extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent)
    {
        if((mPowerManager.isScreenOn())){
            mHandler.postDelayed(mSetStandbyState, 6000);
        }else{
...
//            mIsStandby = true;
    ...
    private class HomletHotPlug implements DisplayHotPlugPolicy
    {
        ...
        {
            //int   hdmiConnect;
            //hdmiConnect = mDisplayManager.getHdmiHotPlugStatus();
            //Slog.d(TAG, "!onTvDacPlugChanged and mIsStandby is = " + mIsStandby + " hdmiConnect = " + hdmiConnect);
            ...
            if(mBootCompleted){
                ...
                if(ypbprIsConnected && mDeviceHasYpbpr)
                    onTvDacYPBPRPlugIn(intent);
                if(!ypbprIsConnected && mDeviceHasYpbpr)
        ...
        private void onTvDacCVBSPlugIn(Intent intent)
        {
            if (SHOW_TVPLUG_IN_CALL) {
                ...
                int curType = mDisplayManager.getDisplayOutputType(0);
                if(1 != hdmiConnect && !mYpbprIsConnected){
                    if(curType != DisplayManagerAw.DISPLAY_OUTPUT_TYPE_TV){
                        mToast.setText(com.android.internal.R.string.cvbs_plug_in);
                        mToast.setOnHideListener(mCVBSPlugInToastOnHide);
                        mToast.setDuration(Toast.LENGTH_SHORT);
                        mToast.show();
                    }
                } else{
                    Slog.d(TAG,"other display device is used, do nothing!");
                }
            }
        }
...
*/
            ...
            if (SHOW_HDMIPLUG_IN_CALL) {
                ...
                if(curType == DisplayManagerAw.DISPLAY_OUTPUT_TYPE_HDMI){
                    ...
                    if(format != null){
                        ...
                        switchDisplayOutput(DisplayManagerAw.DISPLAY_OUTPUT_TYPE_NONE,DisplayManagerAw.DISPLAY_OUTPUT_TYPE_NONE);
                    }
                }else{
        ...
        private void switchDisplayOutput(int type, int format){
            ...
            if(type != DisplayManagerAw.DISPLAY_OUTPUT_TYPE_NONE){
                ...
                mDisplayManager.saveDisplayOutputTypeAndFormat(0,type,format);
            }
            mDisplayManager.setDisplayParameter(0,type,format);
            mDisplayManager.setDisplayMode(DisplayManagerAw.DISPLAY_MODE_SINGLE_FB_GPU);

            if((!mIsStandby) && (type != DisplayManagerAw.DISPLAY_OUTPUT_TYPE_NONE)){
                Slog.d(TAG,"switch audio when screen on and display device change");
                 /* switch audio output mode in the same time */
                AudioManager mAudioManager = (AudioManager)mContext.getSystemService(Context.AUDIO_SERVICE);
                String audioOutputChannelNameOld = Settings.System.getString(mContext.getContentResolver(),
                    Settings.System.AUDIO_OUTPUT_CHANNEL);
                String audioOutputChannelNameNew = null;
                if(audioOutputChannelNameOld == null) {
                    audioOutputChannelNameNew = AudioManager.AUDIO_NAME_CODEC;
                    Slog.d(TAG,"use default audio output channel:" + audioOutputChannelNameNew);
                }else{
                    audioOutputChannelNameNew = audioOutputChannelNameOld;
                }
                Slog.d(TAG, "saved audio channel is " + audioOutputChannelNameNew);
                ArrayList<String> audioOutputChannels = new ArrayList<String>();
                String[] audioList = audioOutputChannelNameNew.split(",");
                for(String audio:audioList){
                    if(!"".equals(audio)){
                        audioOutputChannels.add(audio);
                    }
                }
                Slog.d(TAG, "active the saved audio channels " + audioOutputChannels);
                ArrayList<String> activedChannels = new ArrayList<String>();
                if(audioOutputChannelNameOld.contains("USB")){
                    Slog.d(TAG,"switch contains USB");
                }else{
                     ...
                     (type != DisplayManagerAw.DISPLAY_OUTPUT_TYPE_NONE))){
                         activedChannels.add(AudioManager.AUDIO_NAME_CODEC);
                     }
                     mAudioManager.setAudioDeviceActive(activedChannels,AudioManager.AUDIO_OUTPUT_ACTIVE);
                     Slog.d(TAG,"switch activedChannels.size() = " + activedChannels.size());
                    if(audioOutputChannelNameOld != null){
                        ...
                        Slog.d(TAG,"audio channels change to {" + st + "}");
                    }
                }
            }
        }
        ...
        private OnHideListener mHDMIPlugInToastOnHide = new OnHideListener(){
            public void onHide(){
                ...
                DispList.DispFormat format = DispList.ItemName2Code(databaseValue);
                Slog.d(TAG, "databaseValue = " + databaseValue);
                format = (format.mOutputType == DisplayManagerAw.DISPLAY_OUTPUT_TYPE_HDMI) ? format : DispList.HDMI_DEFAULT_FORMAT;
                switchDisplayOutput(format.mOutputType, format.mFormat);
            }
        };

        private OnHideListener mCVBSPlugInToastOnHide = new OnHideListener(){
            public void onHide(){
                String databaseValue = Settings.System.getString(mContext.getContentResolver(),Settings.System.DISPLY_OUTPUT_FORMAT);
                DispList.DispFormat format = DispList.ItemName2Code(databaseValue);
                Slog.d(TAG, "databaseValue = " + databaseValue);
                format = (format.mOutputType == DisplayManagerAw.DISPLAY_OUTPUT_TYPE_TV) ? format : DispList.CVBS_DEFAULT_FORMAT;
                switchDisplayOutput(format.mOutputType, format.mFormat);
