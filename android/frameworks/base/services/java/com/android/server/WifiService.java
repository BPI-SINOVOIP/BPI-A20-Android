...
public class WifiService extends IWifiManager.Stub {
    ...
    // softap save v1.0
    private static final int WIFI_AP_DISABLED                  = 4;
    private static final int WIFI_AP_ENABLED                   = 5;
    /* Wifi AP enabled while in airplane mode */
    private static final int WIFI_AP_ENABLED_AIRPLANE_OVERRIDE = 6;
    /* Wifi AP disabled due to airplane mode on */
    private static final int WIFI_AP_DISABLED_AIRPLANE_ON      = 7;
    // end
    ...
    // softap save v1.0
    private AtomicInteger mPersistWifiAPState = new AtomicInteger(WIFI_AP_DISABLED);
    // end
    ...
    // softap save v1.0
    private boolean shouldStartWifiAP() {
        mAirplaneModeOn.set(isAirplaneModeOn());
        mPersistWifiAPState.set(getPersistedWifiAPState());
        /* Start if Wi-Fi AP should be enabled or the saved state indicates Wi-Fi AP was on */
        boolean wifiAPEnabled = shouldWifiAPBeEnabledWithBoot();
        return wifiAPEnabled;
    }

    private void SetWifiSaveState(int state) {
        final ContentResolver cr = mContext.getContentResolver();
        Settings.Global.putInt(cr, Settings.Global.WIFI_SAVED_STATE, state);
    }
    // end
    ...
    private int getPersistedWifiAPState() {
        ...
        try {
            ...
            Settings.Global.putInt(cr, Settings.Global.WIFI_AP_ON, WIFI_AP_DISABLED);
    ...
    // softap save v1.0
    private boolean shouldWifiAPBeEnabledWithBoot() {
        if (mAirplaneModeOn.get()) {
            return mPersistWifiAPState.get() == WIFI_AP_ENABLED_AIRPLANE_OVERRIDE;
        } else {
            return mPersistWifiAPState.get() != WIFI_AP_DISABLED;
        }
    }
    // end
    ...
    // softap save v1.0
    private void handleWifiAPToggled(boolean wifiAPEnabled) {
        boolean airplaneEnabled = mAirplaneModeOn.get() && isAirplaneToggleable();
        if (wifiAPEnabled) {
            if (airplaneEnabled) {
                persistWifiAPState(WIFI_AP_ENABLED_AIRPLANE_OVERRIDE);
            } else {
    ...
    public void setWifiApEnabled(WifiConfiguration wifiConfig, boolean enabled) {
        ...
        // softap save v1.0
        Slog.v(TAG, "setWifiApEnabled...");
        long ident = Binder.clearCallingIdentity();
        try {
            handleWifiAPToggled(enabled);
        } finally {
            Binder.restoreCallingIdentity(ident);
        }
        // end
