        ...
        uint32_t minLayerZ, uint32_t maxLayerZ)
{
    ...
    const uint32_t fb_w = hw->getWidth();
    const uint32_t fb_h = hw->getHeight();
    const uint32_t hw_w = hw->setDispProp(DISPLAY_CMD_GETDISPPARA,0,DISPLAY_APP_WIDTH,0);
    const uint32_t hw_h = hw->setDispProp(DISPLAY_CMD_GETDISPPARA,0,DISPLAY_APP_HEIGHT,0);
...
//            sw, sh, minLayerZ, maxLayerZ);
    ...
    glRenderbufferStorageOES(GL_RENDERBUFFER_OES, GL_RGBA8_OES, fb_w, fb_h);
    ...
    if (status == GL_FRAMEBUFFER_COMPLETE_OES) {
        ...
        glViewport(0, fb_h - sh, sw, sh);
        glMatrixMode(GL_PROJECTION);
        glPushMatrix();
        glLoadIdentity();
        glOrthof(0, hw_w, fb_h, fb_h - hw_h, 0, 1);
        glMatrixMode(GL_MODELVIEW);
        ...
        if (glGetError() != GL_NO_ERROR) {
            ...
            if (ptr != MAP_FAILED) {
                ...
                glReadPixels(0, fb_h - sh, sw, sh, GL_RGBA, GL_UNSIGNED_BYTE, ptr);
                if (glGetError() == GL_NO_ERROR) {
                    *heap = base;
                    *w = sw;
                    *h = sh;
...
int SurfaceFlinger::setDisplayParameter(uint32_t cmd,uint32_t  value)
{
    ...
    if (hwc.initCheck() == NO_ERROR)
    {
        ...
        */
        ...
        if(cmd == HWC_LAYER_SETFRAMEPARA && mVideoFramePlayStart){
           repaintEverything();
           mVideoFramePlayStart = false;
           ALOGW("###refresh ui frame only one time, please check me!!!");
        }else if ((cmd == HWC_LAYER_SHOW) && (value == 1)){
           mVideoFramePlayStart = true;
        }
