...
#if (BTHW_DBG == TRUE)
#define BTHWDBG(param, ...) {ALOGE(param, ## __VA_ARGS__);}
#else
...
static uint8_t hw_config_findpatch(char *p_chip_id_str)
{
    ...
    if (strlen(fw_patchfile_name)> 0)
    {
        ...
        ALOGE("FW patchfile: %s", p_chip_id_str);
        return TRUE;
    }
...
static uint8_t hw_config_set_bdaddr(HC_BT_HDR *p_buf)
{
    ...
    ALOGE("Setting local bd addr to %02X:%02X:%02X:%02X:%02X:%02X",
    ...
    if (p_buf != NULL)
    {
        ...
        switch (hw_cfg_cb.state)
        {
            case HW_CFG_SET_UART_BAUD_1:
                ...
                ALOGE("bt vendor lib: set UART baud %i", UART_TARGET_BAUD_RATE);
            ...
            case HW_CFG_DL_FW_PATCH:
                 ...
                 */
                ALOGE("bt vendor lib: set UART baud 115200");
            ...
            case HW_CFG_SET_BD_ADDR:
                ALOGE("vendor lib fwcfg completed");
            ...
            case HW_CFG_READ_BD_ADDR:
                ...
                {
                    ALOGE("Controller OTP bdaddr %02X:%02X:%02X:%02X:%02X:%02X",
...
void hw_config_start(void)
{
    ...
    ALOGE("hw_config_start start send HCI_RESET.");
...
void hw_sco_config(void)
{
    ...
    if (p_buf)
    {
        ...
        *p++ = SCO_PCM_PARAM_SIZE;
        ...
        ALOGE("SCO PCM configure {%d, %d, %d, %d, %d}",
        ...
        *p++ = SCO_I2SPCM_PARAM_SIZE;
        ...
        ALOGE("SCO over I2SPCM interface {%d, %d, %d, %d}",
           bt_sco_param[0], bt_sco_param[1], bt_sco_param[2], bt_sco_param[3]);
#endif
