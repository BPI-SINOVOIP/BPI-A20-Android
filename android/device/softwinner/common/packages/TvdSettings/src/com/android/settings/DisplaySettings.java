...
public class DisplaySettings extends SettingsPreferenceFragment implements
    ...
    private BroadcastReceiver mDisplayOutputChangeListener = new BroadcastReceiver(){
            @Override
            public void onReceive(Context context, Intent intent) {
                String value = Settings.System.getString(getContentResolver(),
										Settings.System.DISPLY_OUTPUT_FORMAT);
                DispList.DispFormat item = null;
                item = DispList.ItemName2Code(value);
                int i = 0;
                for (i=0;i<mOutputModeItems.size();i++){
                    if(item == mOutputModeItems.get(i)){
                        mOutputMode.setValue(Integer.toString(i));
                    }
                }
           }
    };
    ...
    public void onResume() {
        ...
        if (mOutputMode != null) {
            getActivity().registerReceiver(mDisplayOutputChangeListener, new IntentFilter(
               Intent.ACTION_DISPLAY_OUTPUT_CHANGED));
        }
    ...
    public void onPause() {
        ...
        if (mOutputMode != null) {
            getActivity().unregisterReceiver(mDisplayOutputChangeListener);
        }
    }
...
//			pm.setWiseBacklightMode(value);
	...
	private void switchDispMode(DispList.DispFormat item) {
		...
		if (displayManager.getDisplayOutputType(0) != item.mOutputType
            ...
            displayManager.saveDisplayOutputTypeAndFormat(0, item.mOutputType, item.mFormat);
			// 设置音频输出模式 
