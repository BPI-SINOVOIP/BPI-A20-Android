...
static void YUYVToNV12_ASM(const void* yuyv, void *nv21, int width, int height)
{
	unsigned char * pdst_y = (unsigned char *)nv21;
	unsigned char * pdst_uv = (unsigned char *)nv21 + width*height;
	unsigned char * psrc = (unsigned char *)yuyv;
	int  neon_width = width/16;
	int  neon_height = height;
	int i,j;
		...
		if((i&1) == 0)
		{
			for(j=0; j<neon_width;j++)
			{
				...
				asm volatile (
					...
					"vswp           d1,d2        				 \n\t"
					"vst2.8         {d0 - d1}, [%[pdst_y]]!         \n\t"
					"vst2.8         {d2 - d3}, [%[pdst_uv]]!       \n\t"
					: [psrc] "+r" (psrc), [pdst_y] "+r" (pdst_y), [pdst_uv] "+r" (pdst_uv)
					:  //[srcY] "r" (srcY)
...
#ifdef USE_MP_CONVERT
int V4L2CameraDevice::YUYVToYUV420C(const void* yuyv, void *yuv420, int width, int height)
{
	g2d_blt		blit_para;
	int 		err;
...
bool V4L2CameraDevice::captureThread()
{
		...
		&& mCaptureFormat == V4L2_PIX_FMT_YUYV)
	{
		...
		sunxi_flush_cache((void*)(mVideoBuffer.buf_phy_addr[buf.index] | 0x40000000), mFrameWidth*mFrameHeight*2);

		int err = 0;
		err = YUYVToYUV420C((void*)buf.m.offset,
					  (void*)(mVideoBuffer.buf_phy_addr[buf.index] | 0x40000000),
					  mFrameWidth, 
					  mFrameHeight);
		sunxi_flush_cache((void*)buf.m.offset, mFrameWidth*mFrameHeight*2);
		if(err < 0)
		{
			YUYVToNV12_ASM(mMapMem.mem[buf.index],
    ...
    #ifdef  __SUN7I__
		if (mVideoFormat == V4L2_PIX_FMT_NV12)
		{
		...
		else if (mVideoFormat == V4L2_PIX_FMT_NV21)
		{
