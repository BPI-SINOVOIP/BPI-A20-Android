...
static int display_setareapercent(struct display_device_t *dev,int displayno,int percent)
{
    ...
    if(ctx->mode == DISPLAY_MODE_SINGLE_VAR_GPU){
        scn_win.x = 0;//(ctx->width[displayno] - ctx->valid_width[displayno])/2;
        scn_win.y = 0;//(ctx->height[displayno] - ctx->valid_height[displayno])/2;
        scn_win.height = ctx->height[displayno];
        scn_win.width = ctx->width[displayno];
    }else{
...
static int display_setorientation(struct display_device_t *dev,int orientation)
{
    ...
    if(ctx->mode == DISPLAY_MODE_SINGLE_VAR_GPU)
    {
        ...
        layer_para.scn_win.x = 0;//(ctx->width[0] - ctx->valid_width[0])/2;
        layer_para.scn_win.y = 0;//(ctx->height[0] - ctx->valid_height[0])/2;
        layer_para.scn_win.width = ctx->width[0];
        layer_para.scn_win.height = ctx->height[0];
...
static int display_setmode(struct display_device_t *dev,int mode,struct display_modepara_t *para)
{
    ...
    else if(mode == DISPLAY_MODE_SINGLE_VAR_GPU)
    {
        ...
        else if((para->d0type != ctx->out_type[0]) || (para->d0format != ctx->out_format[0]))
        {
            ...
            arg[0] = 0;
            cur_out_type = (__disp_output_type_t)ioctl(ctx->mFD_disp,DISP_CMD_GET_OUTPUT_TYPE,(unsigned long)arg);
            if(para->d0type == 0 && cur_out_type == 0){
                ctx->isStadby[0] = true;
                LOGD("####enter standby = %d",ctx->isStadby[0]);
            }
            LOGD("####cur_out_type:%d",cur_out_type);
            if(cur_out_type == DISP_OUTPUT_TYPE_HDMI && para->d0type == DISPLAY_DEVICE_HDMI)
            {
                arg[0] = 0;
                cur_out_format = ioctl(ctx->mFD_disp,DISP_CMD_HDMI_GET_MODE,(unsigned long)arg);
            ...
            LOGD("####display_setmode:cur_out_type = %d,close_output =%d",cur_out_type, close_output);
            ioctl(ctx->mFD_fb[0], FBIOGET_LAYER_HDL_0, &layer_hdl);
            if (!ctx->isStadby[0] && close_output){
                arg[0] = 0;
                arg[1] = layer_hdl;
                ioctl(ctx->mFD_disp,DISP_CMD_LAYER_CLOSE,(unsigned long)arg);
                display_close_output(dev, 0);
...
static int display_set3dmode(struct display_device_t *dev,int displayno,int cur_3denable)
{
    ...
    else if(1 == cur_3denable && !ctx->cur_3dmode[displayno])
    {
        ...
        if (ctx->mode = DISPLAY_MODE_SINGLE_VAR_GPU){
            layer_para.scn_win.x = left;//(ctx->width[displayno] - ctx->valid_width[displayno])/2 + left;
            layer_para.scn_win.y = 0;//(ctx->height[displayno] - ctx->valid_height[displayno])/2;
            layer_para.scn_win.width = ctx->width[0];
            layer_para.scn_win.height = ctx->height[0];
        } else {
    ...
    else if(0 == cur_3denable && ctx->cur_3dmode[displayno])
    {
        ...
        if (ctx->mode = DISPLAY_MODE_SINGLE_VAR_GPU){
            layer_para.scn_win.x = 0;//(ctx->width[displayno] - ctx->valid_width[displayno])/2;
            layer_para.scn_win.y = 0;//(ctx->height[displayno] - ctx->valid_height[displayno])/2;
            layer_para.scn_win.width = ctx->width[0];
            layer_para.scn_win.height = ctx->height[0];
        }else{
    ...
    if(init_para.b_init)
    {
        ...
        for(sel=0; sel<2; sel++)
        {
                            ...
                            || (init_para.disp_mode == DISP_INIT_MODE_TWO_SAME_SCREEN))))
            {
                ...
                //get input size base on framebuffer
                ...
                //get output size of the screen width and height.
                ctx->width[sel] = display_getwidth(ctx, sel, ctx->out_type[sel], ctx->out_format[sel]);
                ctx->height[sel] = display_getheight(ctx, sel, ctx->out_type[sel], ctx->out_format[sel]);
                ctx->valid_width[sel] = ctx->width[sel];
                ctx->valid_height[sel] = ctx->height[sel];
                ...
                if(sel == 0)
                {
                    ...
                    ioctl(ctx->mFD_fb[0], FBIOGET_LAYER_HDL_0, &layer_hdl);
                    //get layer0 paramaters
                    arg[0] = 0;
                    arg[1] = layer_hdl;
                    arg[2] = (unsigned long)&layer_para;
                    ioctl(ctx->mFD_disp,DISP_CMD_LAYER_GET_PARA,(unsigned long)arg);
                    //set source window size
                    layer_para.src_win.width = ctx->valid_width[sel];
                    layer_para.src_win.height = ctx->valid_height[sel];
                    arg[0] = 0;
                    arg[1] = layer_hdl;
                    arg[2] = (unsigned long)&layer_para;
                    ioctl(ctx->mFD_disp,DISP_CMD_LAYER_SET_PARA,(unsigned long)arg);
                }
