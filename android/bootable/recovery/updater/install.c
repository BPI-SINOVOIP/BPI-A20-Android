...
#endif
#define OTA_STAGE_FILE "/cache/last_ota_stage"
#define SYSTEM_PARTITION "/dev/block/system"
...
Value* MountFn(const char* name, State* state, int argc, Expr* argv[]) {
    ...
    if (strcmp(partition_type, "MTD") == 0) {
        ...
        if (mount(location, mount_point, fs_type,
            ...
            fprintf(stderr, "%s: success to mount %s at %s \n",
                    name, location, mount_point);
            result = mount_point;
        }
    }
...
Value* BurnBootFn(const char* name, State* state, int argc, Expr* argv[]) {
    ...
    if (flash_type==0)
    {
        ...
        if (fd<0)
        {
            printf("device is boot_v1.0 , not update %s \n", boot_num == UBOOT?"boot1":"boot0");
            return StringValue(strdup("t"));
        }
    }
    else if (flash_type==1||flash_type==2)
    {
        int ret = 0;
        ret = checkEmmcUbootType("/dev/block/mmcblk0");
        if(ret == 1)
        {
            printf("device is boot_v1.0 , not update %s \n", boot_num == UBOOT?"boot1":"boot0");
            return StringValue(strdup("t"));
...
Value* AssertBootVersionFn(const char* name, State* state, int argc, Expr* argv[]) {
    ...
    if (flash_type==0)
    {
        ...
        if (fd<0&&ota_boot_version==1.0)
            printf("device is boot_v%s and ota is boot_v%.2f \n", "1.0" , ota_boot_version);
        else if (fd>0&&ota_boot_version==2.0)
            printf("device is boot_v%s and ota is boot_v%.2f \n", "2.0" , ota_boot_version);
        else
            return ErrorAbort(state, "%s() device is boot_v%s , but ota is boot_v%.2f ", name, fd<0?"1.0":"2.0" , ota_boot_version);
        return StringValue(strdup("t"));
    }
    else if (flash_type==1||flash_type==2)
    {
        ...
        if(ret == -1)
        {
            return ErrorAbort(state, "%s() unknow emmc device boot_type ", name);
        }
        else if((ret == 1) && (ota_boot_version == 1.0))
        {
            printf("device is boot_v%s and ota is boot_v%.2f \n", "1.0" , ota_boot_version);
            return StringValue(strdup("t"));
        }
        else if((ret == 2) && (ota_boot_version == 2.0))
        {
            printf("device is boot_v%s and ota is boot_v%.2f \n", "2.0" , ota_boot_version);
            return StringValue(strdup("t"));
        }
        else
            return ErrorAbort(state, "%s() device is boot_v%d , but ota is boot_v%.2f ", name, ret, ota_boot_version);
        }
    else
    {
        return ErrorAbort(state, "%s() unknow boot_type : %d", name, flash_type);
    }
}


Value* CheckSystemFn(const char* name, State* state, int argc, Expr* argv[]) {
        
        static const char *E2FSCK_BIN = "/sbin/e2fsck";
        int status;
        ...
        if (pid > 0) {
            ...
            if (execl(E2FSCK_BIN, E2FSCK_BIN, "-y", SYSTEM_PARTITION, (char *)NULL) < 0)
                 fprintf(stderr,"Cannot run %s error %s \n", E2FSCK_BIN, strerror(errno));
...
Value* MarkOtaStageFn(const char* name, State* state, int argc, Expr* argv[]) {
    if (argc != 1) {
        return ErrorAbort(state, "%s() expects 1 args, got %d", name, argc);
    }

    char* stagestr;
    if (ReadArgs(state, argv, 1, &stagestr) < 0) return NULL;

    int f = open(OTA_STAGE_FILE, O_WRONLY|O_CREAT,0770);
    ...
    if (f!=NULL)
    {
        ...
        {
            int done=0;
            done=write(f, stagestr+writed,to_write-writed);
            if (done<0)
                break;
            writed+=done;
        } 
    ...
    {
        printf("can not open %s, set stage fail!\n",OTA_STAGE_FILE);
    }
    free(stagestr);
    return StringValue(strdup(""));
}
...
void RegisterInstallFunctions() {
    ...
    RegisterFunction("checkSystem",CheckSystemFn);
    RegisterFunction("markOtaStage",MarkOtaStageFn);
}
