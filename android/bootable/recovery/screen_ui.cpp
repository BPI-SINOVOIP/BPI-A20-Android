...
#define CHAR_OFFSET_X (CHAR_WIDTH * 4)
#define CHAR_OFFSET_ROW 1
#define CHAR_OFFSET_Y (CHAR_OFFSET_ROW * CHAR_HEIGHT)
...
void ScreenRecoveryUI::draw_text_line(int row, const char* t) {
  if (t[0] != '\0') {
    gr_text(CHAR_OFFSET_X, (row+1) * CHAR_HEIGHT-1, t);
  }
}
...
void ScreenRecoveryUI::draw_screen_locked()
{
    ...
    if (show_text) {
        ...
        if (show_menu) {
            ...
            gr_fill(0, (menu_top+menu_sel-menu_show_start) * CHAR_HEIGHT + CHAR_OFFSET_Y,
                    gr_fb_width(), (menu_top+menu_sel-menu_show_start+1)*CHAR_HEIGHT+1 + CHAR_OFFSET_Y);

         
            gr_color(255, 255, 255, 255);
            for (i = 0; i < menu_top; ++i) {
                draw_text_line(CHAR_OFFSET_ROW + i, menu[i]);
                               row++;
            }
            ...
            for (i = menu_show_start + menu_top; i < (menu_show_start + menu_top + j); ++i){
                if (i == menu_top + menu_sel) {
                    gr_color(255, 255, 255, 255);
                    draw_text_line(CHAR_OFFSET_ROW + i - menu_show_start, menu[i]);
                    gr_color(64, 96, 255, 255);
                } else {
                    draw_text_line(CHAR_OFFSET_ROW + i - menu_show_start, menu[i]);
                }
            ...
            gr_fill(0, row*CHAR_HEIGHT+CHAR_HEIGHT/2-1 + CHAR_OFFSET_Y,
                   gr_fb_width(), row*CHAR_HEIGHT+CHAR_HEIGHT/2+1 + CHAR_OFFSET_Y);
            // ++i;
