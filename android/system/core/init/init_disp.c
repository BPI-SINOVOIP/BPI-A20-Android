...
int init_initdisplay()
{
    ...
    int loop = 0;
    for(loop = 0;loop < 25 && !hdmistatus;loop++){
        hdmistatus = init_dispgethdmistatus();
        usleep(50000);
    }
    if(hdmistatus)
    {
        format = init_dispgethdmiformat(0);
        init_swtichdisplay(0,DISP_OUTPUT_TYPE_HDMI,format);
    }
    else
    {
        ...
        {
            format = init_dispgettvformat(0);
            init_swtichdisplay(0,DISP_OUTPUT_TYPE_TV,format);
        }
