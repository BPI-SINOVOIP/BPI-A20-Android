...
static int fb_open(struct FB *fb)
{
    ...
    fb->bits = mmap(0, fb_size(fb) * 2, PROT_READ | PROT_WRITE,
                    MAP_SHARED, fb->fd, 0);
...
{
    munmap(fb->bits, fb_size(fb) * 2);
    close(fb->fd);
}

static void fb_set_yoffset(struct FB *fb, __u32 offset)
{
    fb->vi.yoffset = offset;
...
int load_argb8888_image(char *fn)
{
    ...
    INFO("width = %d, height = %d, size = %d\n",image_width, image_height, s.st_size);
    ...
    counth      = image_height;
    linebits    = (unsigned long *)fb.bits + width * height;
    lineptr     = data;
