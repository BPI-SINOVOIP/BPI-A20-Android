...
#include "problem.h"
#include "../version.h"
//e2fsck debug 2013-2-23 by young
#define	E2FSCK_DEBUG_
#ifdef	E2FSCK_DEBUG_

#include <cutils/klog.h>

static int klog_fd = -1;
static int klog_level = 3;
...
void klog_write(int level, const char *fmt, ...)
{
	...
	#define E2F_DBG_INFO(fmt, ...)	\
		klog_write(3, "e2fsck->%s: " fmt "\n", __FUNCTION__, ##__VA_ARGS__)

	#define	E2FSCK_PIPE_DEBUG_
		...
		static int pipe_fd;
		static unsigned char  pipe_msg_temp[MSG_LEN];
	#endif
#else	
	#define E2F_DBG_INFO(fmt, ...)
#endif
...
{
  	pipe_fd = open(PIPE_NAME, O_RDWR | O_NONBLOCK);
  	if(-1 == pipe_fd) {
  		E2F_DBG_INFO("pipe open fail");
    	//exit(FSCK_ERROR);
	}
}

static void pipe_write(int type)
{
	E2F_DBG_INFO("pipe_write type=%i", type);
  	struct cmdpacket cpt;
	memset(&cpt, 0, sizeof(cpt));
	cpt.cmd = type;

	switch (type) {
	case C_IN_START:
		cpt.valid_len = strlen(pipe_msg_temp);
		sprintf(cpt.message, "%s", pipe_msg_temp);
		cpt.dxor = 0;
		break;	
	case C_IN_NEEDFIX:
		cpt.valid_len = strlen(pipe_msg_temp);
	...
	case C_IN_FINISH:
		break;
	default:
		E2F_DBG_INFO("pipe send unknown cmd");
		close(pipe_fd);
		break;
	}  
	if (sizeof(cpt) != write(pipe_fd, &cpt, sizeof(cpt))) {
		E2F_DBG_INFO("write pipe fail");
		close(pipe_fd);
	}
}
...
#ifdef E2FSCK_PIPE_DEBUG_
	if (skip_value)
		pipe_write(C_IN_NEEDFIX);
	pipe_exit();
#endif
	E2F_DBG_INFO("e2fsck_run(ctx)");
