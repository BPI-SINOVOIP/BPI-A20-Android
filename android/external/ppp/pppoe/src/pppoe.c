...
void usage(char const *argv0)
{
	...
	fprintf(stderr,
			...
			"   -M mac         -- Set PADT src mac addr; For random mac host.\n"
...
int main(int argc, char *argv[])
{
	...
	unsigned int hm[6];      /* Mac address in -M option */
...
#ifdef DEBUGGING_ENABLED
	options = "I:VAT:D:hS:C:Usm:np:e:kdf:F:t:M:";
#else
	options = "I:VAT:hS:C:Usm:np:e:kdf:F:t:M:";
#endif
	while((opt = getopt(argc, argv, options)) != -1) {
		switch(opt) {
			case 'M':
				/* Existing MAC addr, just used to kill session with -k and -M option */
				...
				/* Copy MAC address of host */
				for (n=0; n<6; n++) {
					conn.myEth[n] = (unsigned char) hm[n];
				}
				break;
			case 't':
