...
static void session_init(struct sw_hcd *sw_hcd)
{
	int reg_val = 0;
...
static void sw_ed_test_hcd_start(struct sw_hcd *sw_hcd)
{
		...
		const char *buf, size_t count)
{
	...
	sw_ed_test_hcd_start(sw_hcd);

	//print_usb_reg_by_ep(NULL, 0Xf1c13000, 0 ,"EP0");

	 if(!strncmp(buf, "test_j_state", 12)){
		USBC_EnterMode_Test_J(g_sw_hcd_io.usb_bsp_hdle);
		printk("test_mode:%s\n", "test_j_state");

	} else if(!strncmp(buf, "test_k_state", 12)) {
		USBC_EnterMode_Test_K(g_sw_hcd_io.usb_bsp_hdle);
		printk("test_mode:%s\n", "test_k_state");

	} else if(!strncmp(buf, "test_se0_nak", 12)) {
		USBC_EnterMode_Test_SE0_NAK(g_sw_hcd_io.usb_bsp_hdle);
		printk("test_mode:%s\n", "test_se0_nak");
		...
		while((ptr2 = strchr(buf, ',')) != NULL) {
			...
			if(adjust_width > 3){
				printk("adjust amplitude is %d, greater than Max value 3\n", adjust_width);
			}else{
				USBC_Phy_Width_Write(0, 0x20, adjust_width, 2);
				printk("adjust_amplitude:%x\n", (USBC_Phy_Read(0, 0x20, 5) & 0x3));
			}
		}

		fifo = USBC_SelectFIFO(g_sw_hcd_io.usb_bsp_hdle, 0);
		USBC_WritePacket(g_sw_hcd_io.usb_bsp_hdle, fifo, 53, (__u32 *)TestPkt);
		USBC_Dev_WriteDataStatus(g_sw_hcd_io.usb_bsp_hdle, USBC_EP_TYPE_EP0, 0);
		USBC_EnterMode_TestPacket(g_sw_hcd_io.usb_bsp_hdle);
		...
		while((ptr2 = strchr(buf, ',')) != NULL) {
			...
			if(adjust_edge > 7){
				...
				USBC_Phy_Edge_Write(0, 0x20, adjust_edge, 3);
				printk("adjust_slew_rate:%x\n",((USBC_Phy_Read(0, 0x20, 5) >> 2) & 0x7));
			}
		}
		...
		USBC_EnterMode_Idle(g_sw_hcd_io.usb_bsp_hdle);
