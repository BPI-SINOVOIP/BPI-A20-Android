...
static struct proc_dir_entry* port_link = NULL;
static struct proc_dir_entry *pt_root = NULL;
#define PORT_NUM 5
int PORT_NUM_ARRAY[PORT_NUM] = {0,1,2,3,4};
static struct switch_dev *priv_dev;
...
static read_port_link(char *page, char **start, off_t off,
			  int count, int *eof, void *data)
{
    ...
    memset(&link, 0, sizeof(link));
    priv_dev->ops->get_port_link(priv_dev, (int*)data, &link);
    len = sprintf(page, "%d\n", link.link);
    return len;
}
static create_ports_proc_files(char* dir)
{
    int i;

    pt_root = proc_mkdir(dir, NULL);
    for (i=0; i<PORT_NUM; ++i)
    {
        char buf[64];
        sprintf(buf, "port%d_link", i);
        port_link = create_proc_entry(buf, 0444, pt_root);
        port_link->read_proc = read_port_link;
        port_link->data = PORT_NUM_ARRAY[i];
    }
}

static remove_ports_proc_files(char* dir)
{
    int i;
    ...
    for (i=0; i<PORT_NUM; ++i)
    {
        char buf[64];
        sprintf(buf, "port%d_link", i);
        remove_proc_entry(buf, pt_root);
    }
    remove_proc_entry("swconfig", NULL);
}
...
printk("[arlen] phy_flow, %s\n", __func__);
    ...
    priv_dev = dev;
    create_ports_proc_files("swconfig");
	return 0;
}
...
unregister_switch(struct switch_dev *dev)
{
	...
	swconfig_unlock();
	mutex_unlock(&dev->sw_mutex);
    remove_ports_proc_files("swconfig");
}
