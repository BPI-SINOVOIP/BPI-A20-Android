...
/*+++++++++++ arlen added +++++++++++++*/
#if 1
static void kortide_switch_mode_setting(struct b53_device *dev)
{
	unsigned i;
#if flow_log
printk("[arlen] kortide_settings, %s\n", __func__);
#endif

    b53_write8(dev, B53_CTRL_PAGE, 0x8, 0x1c);
	b53_write8(dev, B53_CTRL_PAGE, 0xb, 0x6);
	b53_write8(dev, B53_CTRL_PAGE, 0x60, 0x3);	
#ifdef FIX_100Mbps
	b53_write8(dev, B53_CTRL_PAGE, 0xe, 0x87);
#else
	b53_write8(dev, B53_CTRL_PAGE, 0xe, 0x8b);
#endif
	b53_for_each_port(dev, i) {	
		b53_write16(dev, B53_PVLAN_PAGE, B53_PVLAN_PORT_MASK(i), 
		0x1ff);  //revising enable_port settings
	}
	b53_write16(dev, B53_CTRL_PAGE, 0x12, 0x324); //for LED setting
}
#endif
/*----------- arlen added -------------*/
...
printk("[arlen] b53_flow, %s\n", __func__);
	...
	b53_enable_mib(dev);
	kortide_switch_mode_setting(dev); //arlen added
...
printk("[arlen] b53_flow, %s\n", __func__);
	...
	if (is_cpu_port(priv, port)) {
        ...
        if(is531x5(priv)) {
            ...
            printk("b53_read(page0x%x, addr0x%x) = 0x%x\n", B53_STAT_PAGE, B53_SPEED_STAT, speed);
