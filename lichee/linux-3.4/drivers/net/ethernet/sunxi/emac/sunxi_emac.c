...
static void wemac_shutdown(struct net_device *dev)
{
    ...
    if (db->mos_gpio){
        ...
        __gpio_set_value(db->mos_gpio->gpio.gpio, db->mos_gpio->gpio.data);
        printk("_gpio_set_value(db->mos_gpio->gpio.gpio, 0)\n");
    }
...
extern int sw_get_chip_id(uint8_t *chip_id);

static void gmac_generate_mac_by_chip_id(uint8_t *addr)
{
	...
	// Initialize and obtain chipid
	memset(chipid, 0, sizeof(chipid));
	sw_get_chip_id((uint8_t *)chipid);

	// Apply md5 process to chipid
	...
	crypto_hash_update(&desc, &sg, sizeof(chipid) - 1);
...
static void wemac_get_macaddr(wemac_board_info_t *db)
{
	...
	if (!is_valid_ether_addr(ndev->dev_addr)){
		gmac_generate_mac_by_chip_id(ndev->dev_addr);
	}
