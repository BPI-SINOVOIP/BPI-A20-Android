...
__s32 Disp_lcdc_init(__u32 sel)
{
        ...
        lcdc_clk_on(sel, 0, 0);
        lcdc_clk_on(sel, 0, 1);
        if(gdisp.init_para.boot_output_channel != sel)
        {
            LCDC_init(sel);
            lcdc_clk_off(sel);
        }
        if(gdisp.init_para.boot_output_type == DISP_OUTPUT_TYPE_LCD)
        {
...
__s32 BSP_disp_get_output_mode_width(__u32 sel, __disp_output_type_t type, __u32 mode)
{
	__u32 width = 0;

	if(type == DISP_OUTPUT_TYPE_TV || type == DISP_OUTPUT_TYPE_HDMI)
	{
		width = tv_mode_to_width(mode);
	}
	else if(type == DISP_OUTPUT_TYPE_VGA)
    {
		width = vga_mode_to_width(mode);
    }
    else
    {
		width = gpanel_info[sel].lcd_x;
...
__s32 BSP_disp_get_output_mode_height(__u32 sel, __disp_output_type_t type, __u32 mode)
{
	__u32 height = 0;

    if(type == DISP_OUTPUT_TYPE_TV || type == DISP_OUTPUT_TYPE_HDMI)
	{
		height = tv_mode_to_height(mode);
	}
	else if(type == DISP_OUTPUT_TYPE_VGA)
    {
		height = vga_mode_to_height(mode);
    }
    else
    {
		height = gpanel_info[sel].lcd_y;
