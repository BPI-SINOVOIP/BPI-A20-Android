...
__s32 Display_Hdmi_Init(void)
{
    ...
    if(gdisp.init_para.boot_output_type == DISP_OUTPUT_TYPE_HDMI)
    {
        __u32 sel;
        __disp_tv_mode_t     tv_mode;

        sel = gdisp.init_para.boot_output_channel;
        tv_mode = gdisp.init_para.boot_output_mode;
        gdisp.screen[sel].hdmi_mode = tv_mode;
        gdisp.screen[1 - sel].hdmi_mode = DISP_TV_MOD_720P_60HZ;
        ...
        gdisp.screen[sel].b_out_interlace = Disp_get_screen_scan_mode(tv_mode);
    ...
    {
        gdisp.screen[0].hdmi_mode = DISP_TV_MOD_720P_60HZ;
        gdisp.screen[1].hdmi_mode = DISP_TV_MOD_720P_60HZ;
    }
    return DIS_SUCCESS;
}
...
__s32 BSP_dsip_hdmi_get_input_csc(__u32 sel)
{
    ...
    if(gdisp.init_para.hdmi_get_input_csc)
    {
        ret = gdisp.init_para.hdmi_get_input_csc();
    }
    else
    {
...
__u32 BSP_disp_hdmi_get_hdcp_enable(void)
{
    __s32 ret = -1;

    if(gdisp.init_para.hdmi_get_hdcp_enable)
    {
        ret = gdisp.init_para.hdmi_get_hdcp_enable();
        gdisp.screen[0].hdmi_hdcp_en = ret;
        gdisp.screen[1].hdmi_hdcp_en = ret;
    }
    else
    {
        DE_WRN("hdmi_get_hdcp_enable is NULL\n");
    }
    return ret;
}
...
__s32 BSP_disp_set_hdmi_func(__disp_hdmi_func * func)
{
    ...
    gdisp.init_para.hdmi_get_input_csc = func->hdmi_get_input_csc;
    gdisp.init_para.hdmi_get_hdcp_enable = func->hdmi_get_hdcp_enable;
