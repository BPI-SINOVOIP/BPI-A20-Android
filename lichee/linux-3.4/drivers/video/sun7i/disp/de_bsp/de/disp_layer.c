...
__u32 BSP_disp_layer_get_fb_address(__u32 sel, __u32 hid)
{
    hid = HANDTOID(hid);

    return DE_BE_Layer_Get_Framebuffer(sel, hid);
}

__s32 BSP_disp_layer_get_src_win(__u32 sel, __s32 hid, __disp_rect_t *win)
{
    hid = HANDTOID(hid);

    return DE_BE_Layer_Get_Screen_Win(sel, hid, win);
}
