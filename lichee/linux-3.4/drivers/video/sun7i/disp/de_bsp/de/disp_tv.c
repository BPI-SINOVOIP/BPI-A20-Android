...
__s32 Disp_TVEC_Init(__u32 sel)
{
    ...
    tve_clk_init(sel);
    tve_clk_on(sel);
    if((gdisp.init_para.boot_output_type != DISP_OUTPUT_TYPE_TV  && gdisp.init_para.boot_output_type != DISP_OUTPUT_TYPE_VGA)
                || gdisp.init_para.boot_output_channel != sel)
    {
        TVE_init(sel);
        tve_clk_off(sel);
    }
	...
	if(gdisp.init_para.boot_output_type == DISP_OUTPUT_TYPE_TV
		&& gdisp.init_para.boot_output_channel == sel)
	{
	    gdisp.screen[sel].tv_mode = gdisp.init_para.boot_output_mode;
		...
		gdisp.screen[sel].b_out_interlace = Disp_get_screen_scan_mode(gdisp.init_para.boot_output_mode);
...
__s32 BSP_disp_tv_open(__u32 sel)
{
    if(!(gdisp.screen[sel].status & TV_ON))
    {
        ...
        tcon1_src_select(sel,LCDC_SRC_BLACK);
