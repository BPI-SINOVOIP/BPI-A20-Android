...
__u32 DE_BE_Layer_Get_Framebuffer(__u32 sel, __u8 layidx)
{
	__u32 tmp_l,tmp_h,addr;


    tmp_h = (DE_BE_RUINT32(sel,DE_BE_FRMBUF_HIGH4ADDR_OFF) >> (layidx*8)) & 0xf;
    tmp_l = DE_BE_RUINT32IDX(sel,DE_BE_FRMBUF_LOW32ADDR_OFF,layidx);
    addr = (tmp_h << 29) | (tmp_l >> 3); //phy addr
	addr += 0x40000000;
...
__s32 DE_BE_Layer_Get_Screen_Win(__u32 sel, __u8 layidx, __disp_rect_t * win)
{
    __u32 tmp;

    tmp = DE_BE_RUINT32IDX(sel, DE_BE_LAYER_CRD_CTL_OFF,layidx);
    win->x = tmp & 0xffff;
    win->y = (tmp >> 16) & 0xffff;
    tmp = DE_BE_RUINT32IDX(sel, DE_BE_LAYER_SIZE_OFF,layidx);
    win->width = (tmp & 0x1fff) + 1;
    win->height = ((tmp >> 16) & 0x1fff) + 1;
