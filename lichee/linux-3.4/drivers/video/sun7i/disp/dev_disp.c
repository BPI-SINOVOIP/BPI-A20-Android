...
__u32 suspend_status = 0;//0:normal; suspend_status&1 != 0:in early_suspend; suspend_status&2 != 0:in suspend;
...
__s32 DRV_DISP_Init(void)
{
        ...
        para.boot_output_type = (boot_disp_para >> 16) & 0xff;
        para.boot_output_mode = (boot_disp_para >> 8) & 0xff;
        para.boot_output_channel = (boot_disp_para >> 0) & 0xff;
        if(para.boot_output_type == 0)
        {
            para.boot_output_channel = 0xff; //unsupported
            __wrn("boot output type is none\n");
        }
        g_fbi.boot_output_type = para.boot_output_type;
        g_fbi.boot_output_mode = para.boot_output_mode;
        g_fbi.boot_output_channel = para.boot_output_channel;
