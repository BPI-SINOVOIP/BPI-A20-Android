...
static int Fb_map_boot_logo(__u32 sel, struct fb_info *info)
{
    ...
    hdl = IDTOHAND(0);
    phys_addr = BSP_disp_layer_get_fb_address(sel, hdl);
    BSP_disp_layer_get_src_win(sel, hdl, &win);
    if((phys_addr - 0x40000000 <= 0) || win.width <= 1 || win.height <= 1)
    {
        __wrn("boot logo para error!\n");
        return -EINVAL;
    }

    image_width = win.width;
    image_height = win.height;
    image_size = image_width * image_height * (info->var.bits_per_pixel >> 3);
    output_width = BSP_disp_get_output_mode_width(sel, g_fbi.boot_output_type, g_fbi.boot_output_mode);
    output_height = BSP_disp_get_output_mode_height(sel, g_fbi.boot_output_type, g_fbi.boot_output_mode);
	...
	for(i=0; i<image_height; i++) {
        if(info->var.bits_per_pixel == 32) {
            memcpy((void*)((u32)screen_offset + fb_width*4 * i), image_offset, image_width*4);
            image_offset = (void*)((u32)image_offset + image_width*4);
        }
    }
...
{    
        ...
        if(g_fbi.disp_init.b_init)
        {
                ...
                for(sel = 0; sel<2; sel++)
                {
                        ...
                        ((sel==1) && ((g_fbi.disp_init.disp_mode == DISP_INIT_MODE_SCREEN1) || (g_fbi.disp_init.disp_mode == DISP_INIT_MODE_TWO_DIFF_SCREEN) ||
                                       (g_fbi.disp_init.disp_mode == DISP_INIT_MODE_TWO_SAME_SCREEN))))
                        {
                                if(g_fbi.boot_output_channel == sel && g_fbi.boot_output_type != 0)
                                    continue;
            ...
            if((g_fbi.disp_init.fb_width[i] == 0) || (g_fbi.disp_init.fb_height[i] == 0))
            {
                ...
                if(g_fbi.disp_init.output_type[sel] == DISP_OUTPUT_TYPE_TV || g_fbi.disp_init.output_type[sel] == DISP_OUTPUT_TYPE_HDMI)
                    output_mode = g_fbi.disp_init.tv_mode[sel];
                else if(g_fbi.disp_init.output_type[sel] == DISP_OUTPUT_TYPE_VGA)
                    output_mode = g_fbi.disp_init.vga_mode[sel];
                else
                    output_mode = 0;
                fb_para.width = BSP_disp_get_output_mode_width(sel, g_fbi.disp_init.output_type[sel], output_mode);
                fb_para.height = BSP_disp_get_output_mode_height(sel, g_fbi.disp_init.output_type[sel], output_mode);
            }
            else
            {
    ...
    if(from == 1)
    {
        g_fbi.boot_output_type = 0;
        g_fbi.boot_output_mode = 0;
        g_fbi.boot_output_channel = 0xff;
    }
