...
//#define  LRADC_EN		(1<<0) /* @WadeKu@ value of LRADC_EN is defined by sys_config.fex */
...
/* ===== @WadeKu@ To check if lradc need to enable. Begin ===== */
#include <mach/sys_config.h>

extern script_item_u get_para_value(char* keyname, char* subname);
static int lradc_used;

#define  LRADC_EN		(lradc_used << 0)

static void set_lradc_status(void)
{
    script_item_u	val;

    val = get_para_value("target", "lradc_used");    /* Load lradc_used parameter from sys_config.fex */

    if(val.val == -1) {
        lradc_used = 1;
        dprintk(DEBUG_DATA_INFO, "Failed to load lradc_used parameter, LRADC_EN = %d\n", LRADC_EN);
    }
    else if(val.val == 0) {
        lradc_used = 0;
        dprintk(DEBUG_DATA_INFO, "lradc_used = %d, LRADC_EN = %d\n", lradc_used, LRADC_EN);
    }
    else {
        lradc_used = 1;
        dprintk(DEBUG_DATA_INFO, "lradc_used = %d, LRADC_EN = %d\n", lradc_used, LRADC_EN);
...
/* ===== @WadeKu@ To check if lradc need to enable. End ===== */
...
static int __init swkbd_init(void)
{
    ...
    set_lradc_status(); //@WadeKu@ check if lradc need to enable
