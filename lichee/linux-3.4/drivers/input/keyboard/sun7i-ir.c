...
#define IR_ADDRESS_EXPORT   /* export the ir address to user space */
...
static irqreturn_t ir_irq_service(int irqno, void *dev_id)
{
	...
	if(ir_wakeup){
		tmp = intsta >> 8; 
		if((intsta & 0xff) == 0x12 && (tmp & 0xf) < 0x8 && system_wakestate != 0){
...
#ifdef IR_ADDRESS_EXPORT
static struct dentry *sun7i_ir_debugfs;
static struct dentry *sun7i_ir_addr_node;

static int sun7i_ir_open(struct inode *inode, struct file *filp)
{
    return 0;
}


static ssize_t sun7i_ir_read(struct file *f, char __user *buffer, size_t count, loff_t *ppos)
{
    char addr_str[32];
    ...
    count = sprintf(addr_str, "0x%04x\n", debug_ir_addr);
    if(copy_to_user(buffer, addr_str, count))
        return -EFAULT;
...
static struct file_operations sun7i_ir_fops = {
    .open = sun7i_ir_open,
    .read = sun7i_ir_read
};

static int sun7i_ir_debugfs_init(void)
{
    sun7i_ir_debugfs = debugfs_create_dir("sun7i_ir", NULL);
    if (!sun7i_ir_debugfs) {
        printk(KERN_ERR "create debugfs/sun7i_ir/ failed\n");
        return -1;
    }

    sun7i_ir_addr_node = debugfs_create_file("ir_addr", 0444,
        sun7i_ir_debugfs, NULL, &sun7i_ir_fops);

    if (!sun7i_ir_addr_node) {
        printk(KERN_ERR "create file ir_addr failed\n");
        return -1;
    }
    return 0;
}

static void sun7i_ir_debugfs_remove(void)
{
    if (sun7i_ir_addr_node) {
		debugfs_remove(sun7i_ir_addr_node);
		sun7i_ir_addr_node = NULL;

        debugfs_remove(sun7i_ir_debugfs);
...
static int __init ir_init(void)
{
	...
	//add by fe3o4 <<<
	 ...
	 if(SCIRPT_ITEM_VALUE_TYPE_INT != script_get_item("ir_para", "ir_addr_code", &item)){
	    pr_err("%s: ir_addr_code script_get_item error. \n",__func__ );
	    ir_addr_code = 0x9f00;
	 }else{
        ir_addr_code = item.val;
     }

	printk("sun7i-ir script_get_item,ir_wakeup:%d, power_key:%x,ir_addr_code:%x\n",
        ir_wakeup, (u32)power_key, (u32)ir_addr_code);
	//add end >>>
