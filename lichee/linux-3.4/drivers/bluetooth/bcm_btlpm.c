...
{
	BT_ERR("%s enter.", __FUNCTION__);
	bluesleep_uart_dev = uart_dev;
	BT_ERR("%s device name: %s.", __FUNCTION__, bluesleep_uart_dev->name);
}
...
static int __init bluesleep_probe(struct platform_device *pdev)
{
...
#if 0	 //DK, get uart_dev in sw_uart.c
#if BT_BLUEDROID_SUPPORT
	...
	if(bsi->bt_wake_assert)
		ret = gpio_request_one(bsi->ext_wake, GPIOF_DIR_OUT|GPIOF_INIT_HIGH, NULL);
	else
		ret = gpio_request_one(bsi->ext_wake, GPIOF_DIR_OUT|GPIOF_INIT_LOW, NULL);
	if (ret != 0) {
		BT_ERR("couldn't set bt_wake as output function\n");
		goto free_bsi;
	} 

	ret = gpio_request(bsi->host_wake, "bt_wake_ap_irq");
	if (ret < 0) {
		printk("Failed to request gpio [%d] for BT_HOST_WAKE\n", bsi->host_wake);
		return ret;
	}
