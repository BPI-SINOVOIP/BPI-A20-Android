...
static int ir_addr_code = 0x9f00;
...
unsigned long ir_packet_handler(unsigned char *buf, unsigned long dcnt)
{
	unsigned long len;
...
EXPORT_SYMBOL(ir_packet_handler);
int ir_code_valid(unsigned long code)
{
...
static int __init aw_pm_init(void)
{
    ...
    if(SCIRPT_ITEM_VALUE_TYPE_INT != script_get_item("ir_para", "ir_addr_code", &item)){
        pr_err("%s: script_parser_fetch err. \n", __func__);
        ir_addr_code = 0;
        pr_err("notice: ir_addr_code = %x.\n", ir_addr_code);
    }else{
        ir_addr_code = item.val;
        pr_info("ir_addr_code = %d. \n", ir_addr_code);
    }
