...
static int audio_pa_ctrl_used = 0;
...
static void sun7i_codec_conti_halfdone(dma_hdl_t dma_hdl, void *parg)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		if (play_prtd->params == NULL) {
			...
			*/
			...
			if (0 != sw_dma_ctl(play_prtd->dma_hdl, DMA_OP_SET_FD_CB, (void *)&(play_prtd->play_done_cb))) {
			...
			#else
				memset(&play_prtd->play_done_cb, 0, sizeof(play_prtd->play_done_cb));
				play_prtd->play_done_cb.func = sun7i_codec_conti_halfdone;
				play_prtd->play_done_cb.parg = substream;
				printk("%s, line:%d\n", __func__, __LINE__);
				/*use the full buffer callback, maybe we should use the half buffer callback?*/
				if (0 != sw_dma_ctl(play_prtd->dma_hdl, DMA_OP_SET_FD_CB, (void *)&(play_prtd->play_done_cb))) {
				...
				if (0 != sw_dma_ctl(play_prtd->dma_hdl, DMA_OP_SET_HD_CB, (void *)&(play_prtd->play_done_cb))) {
					printk(KERN_ERR "failed to set dma buffer done!!!\n");
					sw_dma_release(play_prtd->dma_hdl);
					return -EINVAL;
				}
		...
		#if 0
			play_prtd->dma_limit 	=4;
		#else
			play_prtd->dma_limit 	= 1;
		#endif
			play_prtd->dma_period 	= params_period_bytes(params);
			printk("codec::func:%s,line:%d,play_prtd->dma_period :%d\n",__func__,__LINE__,play_prtd->dma_period );
...
{	
   ...
   if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		strcpy(substream->pcm->card->id, "audiocodec");
		#else
		printk("%s, line:%d\n", __func__, __LINE__);
		codec_play_dma_conf.bconti_mode = true;
		codec_play_dma_conf.irq_spt = CHAN_IRQ_FD|CHAN_IRQ_HD;
		strcpy(substream->pcm->card->id, "audiocodec_half");
		#endif
...
{
	printk("[audio codec]:sun7i_codec_shutdown\n");

	if (audio_pa_ctrl_used) {
		item.gpio.data = 0;
		if (0 != sw_gpio_setall_range(&item.gpio, 1)) {
			printk("[audio codec]:sun7i_codec_shutdown sw_gpio_setall_range failed\n");
		}
		msleep(100);
	}
	codec_wr_control(SUN7I_ADC_ACTL, 0x1, PA_ENABLE, 0x0);
...
static int __init sun7i_codec_init(void)
{
	...
	if(SCIRPT_ITEM_VALUE_TYPE_INT != type){
		printk("get audio_used type err!");
	}
	pr_info("value is %d\n", val.val);
    audio_used=val.val;

	type = script_get_item("audio_para", "audio_pa_ctrl_used", &val);
	if(SCIRPT_ITEM_VALUE_TYPE_INT != type){
		printk("get audio_pa_ctrl_used type err!");
	} else {
		pr_info("audio_pa_ctrl_used=%d\n", val.val);
		audio_pa_ctrl_used = val.val;
	}
   if (audio_used) {
