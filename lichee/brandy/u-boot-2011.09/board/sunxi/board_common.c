...
int gpio_control(void)
{
	...
	int gpio_value;
	__u32 gpio_hd;
	user_gpio_set_t	gpio_lte_pwr;

	printf("user_gpio config\n");

	ret = script_parser_fetch("boot_init_gpio", "lte_pwr", (int *)&gpio_lte_pwr, sizeof(user_gpio_set_t) / 4);

	printf("boot_init_gpio gpio_lte_pwr->gpio_name = %s\n", gpio_lte_pwr.gpio_name);
	printf("boot_init_gpio gpio_lte_pwr->port = %d\n", gpio_lte_pwr.port);
	printf("boot_init_gpio gpio_lte_pwr->port_num = %d\n", gpio_lte_pwr.port_num);

	gpio_lte_pwr.mul_sel = 0;
	gpio_hd = gpio_request(&gpio_lte_pwr, 1);
	if (gpio_hd)
	{
		int time;
		gpio_value = 0;
		...
		printf("boot_init_gpio lte_pwr value = %d\n", gpio_value);

		for(time = 0; time < 4; time++)
		{
			gpio_write_one_pin_value(gpio_hd, 1, "lte_pwr");
			__msdelay(5);
		}
...
int check_physical_key_early(void)
{
	...
	__u32 gpio_hd;
	int ret;
	int recovery_type = ONEKEY_SPRITE_RECOVERY_MODE;
	int gpio_value = 0;
  ...
  if (!ret)
	{
		...
		if (gpio_hd)
		{
			...
			if (!gpio_value)
			{
				if (recovery_type == ONEKEY_USB_RECOVERY_MODE)
				{
					gd->key_pressd_value = USB_RECOVERY_KEY_VALUE;
					return 0;
				}
				else if (recovery_type == ONEKEY_SPRITE_RECOVERY_MODE)
				{
...
#ifdef CONFIG_USE_CIR
  	recovery_type = ir_fetch_key();
	if (recovery_type == ONEKEY_USB_RECOVERY_MODE)
	{
		gd->key_pressd_value = USB_RECOVERY_KEY_VALUE;
		return 0;
