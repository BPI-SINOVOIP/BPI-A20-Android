...
static int init_layer(void)
{
	...
	args[0] = 0;
	fb_para.width = ioctl(disp, DISP_CMD_SCN_GET_WIDTH, args);
	fb_para.height = ioctl(disp, DISP_CMD_SCN_GET_HEIGHT, args);
...
int main(int argc, char *argv[])
{
	...
	__disp_tv_output_t tv_output_type;
	int mic_activated;
	...
	ret = init_layer();
	if (ret < 0) {
		db_error("tvtester: init layer failed\n");
		goto err;
	}
	...
	while (1) {
		...
		tv_output_type = (__disp_tv_output_t)ioctl(disp, DISP_CMD_TV_GET_INTERFACE,(void*)args);
		//db_warn("tv_output_type[%d]\n", tv_output_type);
		if(DISP_TV_CVBS == tv_output_type){
			...
			*/
			...
			if((dev_status == 1)
				|| (disp_output_type == DISP_OUTPUT_TYPE_TV)){
				args[0] = 0;
