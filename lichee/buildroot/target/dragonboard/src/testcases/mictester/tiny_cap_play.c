...
int setup_mixer(int mixer_num)
{
   ...
   if (script_fetch("mic", "volume", &volume, 1) || 
    ...
    //tinymix_set_value(mixer, 1, 1);//mic output volume
   // tinymix_set_value(mixer, 2, 0);//fm output volume               
	tinymix_set_value(mixer, 3, 1);//line output volume    
	tinymix_set_value(mixer, 8, 1);//ext pa  on     
	tinymix_set_value(mixer, 6, mic2_gain);//mic2 amp 
	tinymix_set_value(mixer, 7, mic1_gain);//mic1 amp 
